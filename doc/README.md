...
This document overviews the USG python module, build tooling, debian maintainer scripts and other important files in this repository.

## USG core functionality
...
The core functionality of usg is in `src/usg/`:
...
└── usg
    ...
    ├── utils.py             # utility functions like gunzipping, permission validation, etc
    ├── constants.py         # constants
    ├── exceptions.py        # custom exceptions
    └── version.py           # version
```
The implementation details are described in more detail later.

## Project configuration and data files

```sh
pyproject.toml               # project configuration and metadata (Ubuntu 24.04 +)
setup.cfg                    # project configuration and metadata (Ubuntu 22.04)
...
The pybuild backend is used to build the python package, based on the metadata in `pyproject.toml` (doesn't work
in jammy due to setuptools<61.0 -> uses setup.cfg instead). Pybuild automatically runs the tests during build.

The python package `usg` is installed as a *private package* to `/usr/share/usg`.

The legacy `usg-benchmarks-N` binary package was replaced with `usg-benchmarks`, which now contains all the available benchmarks in gzipped format, together with the metadata file `benchmarks.json`, which is generated by process_benchmarks.py. The benchmarks are installed to `/usr/share/usg-benchmarks`.
...
- `tests/`: pytest tests covering src/usg
- `tools/tests/`: pytest tests covering tools
- `debian/tests/`: E2E autopkgtests
...
Rel(usg, profile,  "Contains")
Rel(usg, tailoring, "Loads")

Rel(profile, benchmark, "")
Rel(benchmark, profile, "")
Rel(profile, tailoring, "")
