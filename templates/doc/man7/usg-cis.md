% USG-CIS(7) usg-benchmarks <<USG_BENCHMARKS_VERSION_PLACEHOLDER>>
...
However, there is one exception to this: rule 5.4.1.5 (Ensure inactive password lock is configured) applies to all login accounts, including the ones
...
Use the variable var\_apparmor\_mode to select one these values according to site requirements:
 ...
 - enforce: (default) set all Apparmor profiles in /etc/apparmor.d to enforce mode
 - complain: set all Apparmor profiles in /etc/apparmor.d to complain mode
 - keep_existing_mode: Do not change existing modes of Apparmor profiles.
...
The value must be a comma-separated list of servers. The default values are the safe ones recommended by the cis benchmark.
...
- 1.1.2.1.1	   Ensure /tmp is a separate partition 	 (usg rules:partition_for_tmp)
- 1.1.2.2.1	   Ensure /dev/shm is a separate partition 	 (usg rules:partition_for_dev_shm)
...
- 1.4.1	   Ensure bootloader password is set 	 (usg rules:grub2_password,grub2_uefi_password)
- 2.1.21   Ensure mail transfer agent is configured for local-only mode 	 (usg rules:has_nonlocal_mta)
- 4.1.3	   Ensure ufw service is enabled 	 (usg rules:check_ufw_active)
- 4.1.6	   Ensure ufw firewall rules exist for all open ports 	 (usg rules:ufw_rules_for_open_ports)
- 4.1.7	   Ensure ufw default deny firewall policy 	 (usg rules:set_ufw_default_rule)
- 4.2.8	   Ensure nftables default deny firewall policy 	 (usg rules:nftables_ensure_default_deny_policy)
- 4.3.2.1	   Ensure iptables default deny firewall policy 	 (usg rules:set_iptables_default_rule)
- 4.3.2.4	   Ensure iptables firewall rules exist for all open ports 	 (usg rules:iptables_rules_for_open_ports)
- 4.3.3.1	   Ensure ip6tables default deny firewall policy 	 (usg rules:set_ip6tables_default_rule)
- 4.3.3.4	   Ensure ip6tables firewall rules exist for all open ports 	 (usg rules:ip6tables_rules_for_open_ports)
...
- 5.4.2.3	   Ensure group root is the only GID 0 group 	 (usg rules:groups_no_zero_gid_except_root)
- 5.4.2.4	   Ensure root password is set 	 (usg rules:ensure_root_access_controlled)
- 5.4.2.5	   Ensure root path integrity 	 (usg rules:accounts_root_path_dirs_no_write,root_path_all_dirs,root_path_no_dot,no_dirs_unowned_by_root)
- 5.4.2.8	   Ensure accounts without a valid login shell are locked 	 (usg rules:no_invalid_shell_accounts_unlocked)
- 6.3.1.3	   Ensure auditing for processes that start prior to auditd is enabled 	 (usg rules:zipl_audit_argument)
- 6.3.1.4	   Ensure audit_backlog_limit is sufficient 	 (usg rules:zipl_audit_backlog_limit_argument)
...
- 7.2.5	   Ensure no duplicate UIDs exist 	 (usg rules:account_unique_id)
...
# INTERNET RESOURCES
Ubuntu 22.04 CIS Benchmark: https://workbench.cisecurity.org/benchmarks/17074
