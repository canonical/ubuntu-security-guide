...
from usg.models import Benchmark, Profile, Benchmarks
from usg.exceptions import ProfileNotFoundError, BenchmarkError
...
def test_usgbenchmarks_from_missing_benchmarks(tmp_path, dummy_benchmarks):
    ...
    with pytest.raises(BenchmarkError, match="Invalid '.*' contents. Could not find key 'benchmarks'"):
...
def test_usgbenchmarks_from_missing_version(tmp_path, dummy_benchmarks):
    ...
    json_file = tmp_path / "benchmarks.json"
    json_file.write_text(json.dumps(b))
    with pytest.raises(BenchmarkError, match="Invalid '.*' contents. Could not find key 'version'"):
        Benchmarks.from_json(json_file)
...
def test_usgbenchmarks_from_duplicate_benchmark_id(tmp_path, dummy_benchmarks):
    ...
    json_file = tmp_path / "benchmarks.json"
    json_file.write_text(json.dumps(b))
    with pytest.raises(BenchmarkError, match="Malformed dataset - duplicate benchmark ID: ubuntu2404_CIS_1"):
        Benchmarks.from_json(json_file)
...
def test_usgbenchmark(dummy_benchmarks):
    ...
    assert benchmark.description == "ComplianceAsCode implementation of CIS Ubuntu 24.04 LTS v1.0.1 benchmark.\n"
    assert benchmark.compatible_versions == ["v1.0.0.usg1"]
    assert benchmark.breaking_upgrade_path == ["v2.0.0"]
    assert benchmark.release_notes_url == "https://github.com/canonical/ubuntu-security-guide/"
    assert benchmark.reference_url == "https://www.cisecurity.org/benchmark/ubuntu_linux/"
    ...
    assert t_rel_path == Path("ubuntu2404_CIS_2/tailoring/cis_level2_server-tailoring.xml")
    ...
    assert ds_file.sha256 == "5ef3b9feb87381e7cca99f50cfaae6d5789ba66e512e97bd82e027f2005a8366"
...
def test_usgbenchmark_bad_data(dummy_benchmarks, missing_key):
    ...
    with pytest.raises(BenchmarkError, match=f"Failed to create Benchmark object from.*: '{missing_key}"):
...
def test_usgbenchmark_tailoring_file(dummy_benchmarks):
    benchmarks = Benchmarks.from_json(dummy_benchmarks)
    benchmark = benchmarks["ubuntu2404_CIS_1"]
    assert benchmark.get_tailoring_file_relative_path("cis_level1_server") == Path("ubuntu2404_CIS_1/tailoring/cis_level1_server-tailoring.xml")
    assert benchmark.get_tailoring_file_relative_path("cis_level2_server") == Path("ubuntu2404_CIS_1/tailoring/cis_level2_server-tailoring.xml")
...
def test_usgbenchmark_profiles(dummy_benchmarks):
    ...
    for profile_id, profile in benchmarks["ubuntu2404_CIS_1"].profiles.items():
        ...
        assert profile.profile_legacy_id == benchmark.profiles[profile_id].profile_legacy_id
