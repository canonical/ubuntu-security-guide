...
MANUAL_RULES = {
    'common': {'1.1.19', '1.1.20', '1.1.21', '1.2.1', '1.2.2', '1.9', '2.2.1.2',
               '2.4', '3.5.1.5', '3.5.1.6', '3.5.2.3', '3.5.2.7', '3.5.3.2.3',
               '3.5.3.3.3', '3.5.3.3.4', '4.2.1.3', '4.2.1.6', '4.3', '5.5',
               '6.1.13', '6.1.14'},
    'Level_1_Server': set(),
...
def pretty_print_bash_section(profile_rule_mapping):
    ...
    common_line = "\\\n".join(textwrap.wrap(common_line, width=78, subsequent_indent=' '*8))
    print(f"{common_line}")
    for profile_id in PROFILE_ORDER:
        profile_name = PROFILE_ID_TO_NAME[profile_id]
        ref = "$common"
        ...
        joined_rules = join_rules(profile_rule_mapping[profile_id].difference(basis))
        line = f"    rulehash[{profile_name}]=\"{ref}\""
        if joined_rules:
            line += f"\" {joined_rules}\""
        line = "\\\n".join(textwrap.wrap(line, width=78, subsequent_indent=' '*8))
        print(f"{line}")
